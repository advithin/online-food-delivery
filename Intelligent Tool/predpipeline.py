...
print(X_train_prepared.shape[1] == X_test_prepared.shape[1])
...
#Random Forest
forest_rmse = calc_rmse(RandomForestRegressor(), X_train_prepared, y_train, cv=5)
print(f'Forest Regression rmse with cv=5 without any hyperparemeter configuration: {forest_rmse}')
...
test_data.append(("No","No",0,300.0,"Delivery","Banashankari"))

test_df=pd.DataFrame(test_data,columns=['online_order','book_table','votes','approx_cost(for two people)','listed_in(type)','listed_in(city)'])

test_num=test_df.loc[:, num_attribs]
test_cat =test_df.loc[:, cat_attribs]
test_check=full_pipeline.transform(test_df)

predictions=final_model.predict(test_check)
